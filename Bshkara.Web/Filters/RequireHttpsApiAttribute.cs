...
namespace Bshkara.Web.Filters
{
    public class RequireHttpsApiAttribute : AuthorizationFilterAttribute
    {
        ...
        {
            if (actionContext.Request.RequestUri.Scheme != Uri.UriSchemeHttp && !IsForwardedSsl(actionContext))
            {
        ...
        private static bool IsForwardedSsl(HttpActionContext actionContext)
        {
            ...
            var forwardedSsl = xForwardedProto.Value != null &&
                               xForwardedProto.Value.Any(
                                   x => string.Equals(x, "https", StringComparison.InvariantCultureIgnoreCase));
            return forwardedSsl;
        }
    }
}