...
namespace Bshkara.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var agencyPackagesService = DependencyResolver.Current.GetService<AgencyPacksService>();
            ViewBag.PackageId =
                new SelectList(
                    agencyPackagesService.GetPackageIdsValues(), "id", "value");
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new UserEntity {UserName = model.Email, Email = model.Email, Name = model.Agency.Name.En};
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    await UserManager.AddToRoleAsync(user.Id, RoleEntity.AgentAdminRoleName);
                    ...
                    //Create Agency
                    var agencyService = DependencyResolver.Current.GetService<AgenciesService>();
                    agencyService.InsertOrUpdate(model.Agency);
                    ...
                    var agencyPacksService = DependencyResolver.Current.GetService<AgencyPacksService>();
                    agencyPacksService.InsertOrUpdate(new AgencyPackageEntity
                    {
                        AgencyId = model.Agency.Id,
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ViewBag.UserNotExists = false;
                var user = await UserManager.FindByNameAsync(model.Email);
                if (user == null /*|| !await UserManager.IsEmailConfirmedAsync(user.Id)*/)
                {
                    ModelState.AddModelError("Mail", $"Can't fine user with {model.Email} email");
                    return View(model);
                }
